...
class MultibotWaypointCostUpdater(Node):
    def __init__(self):
        super().__init__('multibot_waypoint_cost_updater')
        ...
        # Cache of current costs to avoid unnecessary updates
        self.current_costs = {}
        ...
        # Create client for the cost update service
        ...
        # Subscribe to prediction grid with latest message QoS
        ...
        # Subscribe to waypoint graph to get current waypoint positions
        self.waypoints = {}
        ...
        # Subscribe to flow grid update complete notifications
        ...
        # Publisher for waypoint cost update complete notification
        ...
        # Timer for periodic updates (as fallback)
        update_period = 1.5/self.base_update_frequency
        self.update_timer = self.create_timer(update_period, self.update_timer_callback)
        
        # Timer for periodically checking if waypoints are in obstacles
        self.map_check_timer = self.create_timer(self.map_check_interval, self.check_waypoints_in_obstacles)
        
        self.get_logger().info('Multibot Waypoint Cost Updater Node initialized')
        self.get_logger().info(f'Parameters: min_cost={self.min_cost}, max_cost={self.max_cost}, prediction_threshold={self.prediction_threshold}')
        ...
        if 0 <= grid_x < width and 0 <= grid_y < height:
            ...
            if hasattr(self, '_log_counter'):
                ...
                if self._log_counter % 50 == 0:
                    self.get_logger().warn(f"Coordinates outside map bounds. World: ({x:.2f}, {y:.2f})")
            else:
                self._log_counter = 0
                self.get_logger().warn(f"Coordinates outside map bounds. World: ({x:.2f}, {y:.2f})")
            return None, None
        ...
        if 0 <= index < len(self.map_data.data):
            ...
            if hasattr(self, '_index_log_counter'):
                ...
                if self._index_log_counter % 50 == 0:
                    self.get_logger().warn(f"Invalid map index: {index}, grid: ({grid_x}, {grid_y})")
            else:
    ...
    def find_waypoints_in_prediction_grid(self):
        ...
        if waypoints_with_pred > 0:
            self.get_logger().info(f"Found {waypoints_with_pred}/{waypoints_in_grid} waypoints with significant predictions")
    ...
    def update_waypoint_cost(self, node_id, cost):
        ...
        if node_id == -1:
            ...
            self.get_logger().info(f'Bulk updating {updated_waypoints} waypoints, max cost: {max_cost:.2f}')
        ...
        try:
            ...
            if response.success:
                if isinstance(updated_count, int) and max_cost is not None:
                    ...
                    self.get_logger().debug(f'Successfully updated cost for node {node_id}')
            else:
...
def main(args=None):
    ...
    node = MultibotWaypointCostUpdater()
    try:
        rclpy.spin(node)
