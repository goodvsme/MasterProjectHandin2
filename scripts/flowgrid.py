...
class FlowGrid(Node):
    def __init__(self):
        super().__init__('flow_grid')
        
        self.declare_parameter('num_directions', 8)  
        self.declare_parameter('use_fixed_resolution', True)  
        self.declare_parameter('fixed_resolution', 1.0)  
        self.declare_parameter('prediction_threshold', 0.01)  
        self.declare_parameter('prediction_concentration', 2.0)  
        self.declare_parameter('max_propagation_distance', 4)  
        self.declare_parameter('distance_decay_factor', 0.95)  
        self.declare_parameter('prediction_scale_factor', 1)  
    ...
    def world_to_grid(self, x, y):
        ...
        grid_y = int((self.origin_y + self.height * self.resolution - y) / self.resolution)
    ...
    def is_cell_occupied(self, grid_x, grid_y):
        ...
        if self.use_fixed_resolution and self.fixed_resolution != self.original_map.info.resolution:
            ...
            world_y = self.origin_y + self.height * self.resolution - grid_y * self.resolution
            
            map_x = int((world_x - self.origin_x) / map_res)
            map_y = int((self.origin_y + self.original_map.info.height * map_res - world_y) / map_res)
    ...
    def save_flow_grid_image(self):
        ...
        try:
            ...
            filename = f"flow_grid_{timestamp}.png"
            ...
            plt.suptitle(f'Flow Grid State at {time_str}', fontsize=16)
            plt.tight_layout() 
            ...
            try:
                ...
                plt.text(0.5, 0.5, f"Error saving image: {str(e)}", 
                        horizontalalignment='center', verticalalignment='center')
                error_path = os.path.join(self.save_path, f"flow_grid_error_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
