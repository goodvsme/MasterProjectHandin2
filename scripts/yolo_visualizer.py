...
class HumanDetectorWithOrientation(Node):
    def __init__(self):
        ...
        self.camera_params = {
            'left': {
                ...
                'min_depth': 0.1,
                'max_depth': 10.0,
                'frame_id': 'left_camera_depth_optical_frame',
                'rotated_width': None,
                'rotated_height': None
            },
            'right': {
                ...
                'min_depth': 0.1,
                'max_depth': 10.0,
                'frame_id': 'right_camera_depth_optical_frame',
                'rotated_width': None,
                'rotated_height': None
            }
        }
        ...
        self.left_sub = self.create_subscription(
            Image, '/camera/left_camera/color/image_raw',
            lambda msg: self.image_callback(msg, 'left'), qos_profile)
        self.right_sub = self.create_subscription(
            Image, '/camera/right_camera/color/image_raw',
            lambda msg: self.image_callback(msg, 'right'), qos_profile)
        self.left_depth_sub = self.create_subscription(
            Image, '/camera/left_camera/depth/image_raw',
            lambda msg: self.depth_callback(msg, 'left'), qos_profile)
        self.right_depth_sub = self.create_subscription(
            Image, '/camera/right_camera/depth/image_raw',
            lambda msg: self.depth_callback(msg, 'right'), qos_profile)

        self.detections_pub = self.create_publisher(PoseArray, '/raw_human_detections', 10)
        
        self.get_logger().info('Human detector with orientation inference started - publishing to /raw_human_detections')
...
def main(args=None):
    ...
    node = HumanDetectorWithOrientation()
